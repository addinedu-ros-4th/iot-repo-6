...
#### 추후 문제가 더 발생할 수도 있지만, 다음 기수에서 더 완성도 높은 스마트 스위치를 개발해줄 것이라고 생각합니다.

#### P.S. 기존의 ESP32.ino 코드에는 프린트와 딜레이가 있습니다. 프린트를 없애고 밀리스 함수로 변환하는 것이 좋을 것 같습니다. 또한, ESP32가 아닌 가벼운 아두이노로 구현해 보는 것도 좋을 것 같습니다.

## 코드 실행방법
#### classroom_led_control_ESP32 다운 및 실행
#### LIBRARY MANAGER -> IRremote 3.6.0 install(최신 버전으로 설치하면 리모컨이 고정된 16진수 코드를 수신 못하는 문제가 발생)
