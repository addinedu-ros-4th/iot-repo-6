...
class MainWindow(QMainWindow):
    def __init__(self):
       ...
       # 초기에 버튼과 라벨 숨김
        ...
        self.hum_values = []   # 습도 저장을 위한 리스트
        self.air_values = []   # 공기 저장을 위한 리스트
        self.visualized_tem = False
        self.visualized_hum = False  
        self.visualized_air = False    
        ...
        self.ui.graphAIR.clicked.connect(self.visualizeAir)
        self.ui.graphHUM.clicked.connect(self.visualizeHum)
        ...
        self.timer.start(100)  # 1초에 한 번씩 타이머가 작동
           # 타이머 생성 및 설정
    ...
    def Recv(self, message):
        ...
        self.time_values.append(formatted_time)
    ...
    def visualizeTem(self):
        ...
        if self.visualized_tem:
            ...
            # Ensure that the sizes of time_values and tem_values are the same
            min_length = min(len(self.time_values), len(self.tem_values))
            self.time_values = self.time_values[:min_length]
            self.tem_values = self.tem_values[:min_length]

            self.line_tem, = self.ax_tem.plot(self.time_values, self.tem_values)

            # x 축 레이블을 제거
            self.ax_tem.set_xticks([])

            plt.tight_layout()
            self.fig_tem.canvas.mpl_connect('close_event', self.handle_close)

    def visualizeHum(self):
        self.visualized_hum = True
        if self.visualized_hum:
            plt.ion()
            if self.fig_hum is None:
                self.fig_hum, self.ax_hum = plt.subplots()
                plt.xlim([0, 300])
                plt.ylim([20, 70])
                self.ax_hum.set_ylabel('Humidity (%)')
                self.ax_hum.set_xlabel('Time')
                self.ax_hum.set_title('Real-time Humidity Visualization')
                self.ax_hum.legend()

            # Ensure that the sizes of time_values and hum_values are the same
            min_length = min(len(self.time_values), len(self.hum_values))
            self.time_values = self.time_values[:min_length]
            self.hum_values = self.hum_values[:min_length]

            line_hum, = self.ax_hum.plot(self.time_values, self.hum_values, label='Humidity')

            # x 축 레이블을 제거
            self.ax_hum.set_xticks([])

            plt.tight_layout()
            self.fig_hum.canvas.mpl_connect('close_event', self.handle_close)

    def visualizeAir(self):
        self.visualized_air = True
        if self.visualized_air:
            plt.ion()
            if self.fig_air is None:
                self.fig_air, self.ax_air = plt.subplots()
                plt.xlim([0, 300])
                plt.ylim([100, 170])
                self.ax_air.set_xlabel('Time')
                self.ax_air.set_ylabel('Air (PPM)')
                self.ax_air.set_title('Real-time Air Visualization')
                self.ax_air.legend()

            # Ensure that the sizes of time_values and air_values are the same
            min_length = min(len(self.time_values), len(self.air_values))
            self.time_values = self.time_values[:min_length]
            self.air_values = self.air_values[:min_length]

            self.line_air, = self.ax_air.plot(self.time_values, self.air_values)

            # x 축 레이블을 제거
            self.ax_air.set_xticks([])

            plt.tight_layout()
            self.fig_air.canvas.mpl_connect('close_event', self.handle_close)
    ...
    def handle_close(self, event):
        ...
        if event.canvas.figure == self.fig_hum:
            self.visualized_hum = False
            self.fig_hum = None
            self.line_hum = None
            self.time_values = []
        ...
        if event.canvas.figure == self.fig_air:
            self.visualized_air = False
            self.fig_air = None
            self.line_air = None
            self.time_values = []
    ...
    def closeEvent(self, event):
        ...
        if self.fig_hum is not None:
            self.visualized_tem = False
            self.fig_hum.canvas.mpl_disconnect(self.fig_hum.canvas.manager.key_press_handler_id)
            self.fig_hum.clear()
            plt.close(self.fig_hum)

        if self.fig_air is not None:
            self.visualized_tem = False
            self.fig_air.canvas.mpl_disconnect(self.fig_air.canvas.manager.key_press_handler_id)
            self.fig_air.clear()
            plt.close(self.fig_air)
